        ...
        if (isset($response['order_urls']) && count($response['order_urls']) > 0) {
            ...
            foreach ($response['order_urls'] as $jetorderurl) {
               ...
               /*$email=$resultObject->hash_email;
                ...
                /*2017/07/18 dini add start*/
                $email = 'jetorders@kbethos.com';
                 $firstname = 'Name not Available ';
                $lastname = 'Please Check on Jet for this Customer';
 ...
 * */
	...
	public function updateInvcron(){
        ...
        if(count($batch_count) == 0)
            {
                $response = Mage::helper('jet')->CGetRequest('/portal/merchantskus?from=0&size=100000');
                $result = json_decode($response,true);
